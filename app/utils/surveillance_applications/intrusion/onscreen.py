...
from utils.surveillance_applications.intrusion.checks import check_font, check_version, is_ascii
from utils.surveillance_applications.intrusion.files import increment_path
...
class Annotator2:
    """
    Ultralytics Annotator2 for train/val mosaics and JPGs and predictions annotations.
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        """Initialize the Annotator2 class with image and line width along with color palette for keypoints and limbs."""
        assert im.data.contiguous, "Image not contiguous. Apply np.ascontiguousarray(im) to Annotator2() input images."
    ...
    def count_labels(self, counts=0, count_txt_size=2, color=(255, 255, 255), txt_color=(0, 0, 0)):
        ...
        text_y = t_size_in[1] + t_size_in[1] + t_size_in[1] + t_size_in[1]
...
def plot_images(
    ...
    Annotator2 = Annotator2(mosaic, line_width=round(fs / 10), font_size=fs, pil=True, example=names)
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        Annotator2.rectangle([x, y, x + w, y + h], None, (255, 255, 255), width=2)  # borders
        if paths:
            Annotator2.text((x + 5, y + 5), text=Path(paths[i]).name[:40], txt_color=(220, 220, 220))  # filenames
        if len(cls) > 0:
            idx = batch_idx == i
            ...
            if len(bboxes):
                ...
                for j, box in enumerate(boxes.astype(np.int64).tolist()):
                    ...
                    if labels or conf[j] > 0.25:  # 0.25 conf thresh
                        ...
                        Annotator2.box_label(box, label, color=color, rotated=is_obb)

            elif len(classes):
                for c in classes:
                    color = colors(c)
                    c = names.get(c, c) if names else c
                    Annotator2.text((x, y), f"{c}", txt_color=color, box_style=True)
            ...
            if len(masks):
                ...
                im = np.asarray(Annotator2.im).copy()
                ...
                Annotator2.fromarray(im)
    if not save:
        return np.asarray(Annotator2.im)
    Annotator2.im.save(fname)  # save
    if on_plot:
        on_plot(fname)
